...
const apps: App[] = [
  ...
  { id: 'google', title: 'Google', subtitle: 'Fixed • No captcha', description: 'Google results via private proxy — no recaptcha, instant. Real Google, school-proof.', mark: 'G', color: 'cookie', path: '/app-frames/google/index.html', genre: 'Search' },
  ...
  { id: 'nowgg', title: 'now.gg', subtitle: 'Cloud gaming hub.', description: 'now.gg cloud — play any mobile game in browser, unblocked.', mark: 'NG', color: 'stack', path: '/app-frames/nowgg/index.html', genre: 'Gaming' },
    ...
    <main className="lounge-shell" style={{background:'radial-gradient(800px 600px at 10% -10%, rgba(125,107,255,.12), transparent 60%), radial-gradient(700px 500px at 90% 0%, rgba(215,243,74,.10), transparent 60%), radial-gradient(600px 400px at 50% 100%, rgba(255,92,92,.06), transparent 60%), var(--background)'}}>
      ...
      <section className="hero" id="top" style={{paddingBottom:32, minHeight:520, position:'relative', overflow:'hidden'}}>
        ...
        <div className="hero-copy" style={{position:'relative', zIndex:1}}>
          ...
          <p className="hero-text" style={{maxWidth:520, fontSize:15, lineHeight:1.6}}>YouTube <strong style={{color:'#22c55e'}}>actually plays</strong> in the lounge • Google <strong style={{color:'#4285F4'}}>never shows recaptcha</strong> • TikTok <strong style={{color:'#ff2e63'}}>scrolls smooth</strong>. No installs, no blocks, just tap.</p>
          ...
          <div style={{display:'flex',gap:8, flexWrap:'wrap', marginTop:16}}>
            ...
            <span style={{display:'flex',alignItems:'center',gap:6, padding:'6px 12px', borderRadius:999, background:'rgba(66,133,244,.10)', border:'1px solid rgba(66,133,244,.2)', color:'#8ab4ff', fontSize:11, fontWeight:800}}><ShieldCheck size={12}/> Google FIXED — no recaptcha</span>
      ...
      <section style={{padding:'0 18px', maxWidth:1280, margin:'0 auto', width:'100%'}}>
        <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(280px, 1fr))', gap:12, marginTop: -8}}>
          {[youtubeFixed, googleFixed, tiktokFixed].map(a=> (
            <div key={a.id} onClick={()=> setActive(a)} role="button" style={{padding:16, borderRadius:16, border:'1px solid rgba(255,255,255,.08)', background:'linear-gradient(135deg, rgba(255,255,255,.05), rgba(255,255,255,.02))', display:'flex', gap:12, alignItems:'center', cursor:'pointer', backdropFilter:'blur(8px)', transition:'transform .2s'}}>
              <span style={{width:44,height:44,borderRadius:12, background: a.id==='youtube'? '#ff2e63' : a.id==='google'? '#4285F4' : '#000', display:'grid', placeItems:'center', color:'#fff', fontWeight:900, boxShadow:'0 8px 20px rgba(0,0,0,.25)'}}>{a.mark}</span>
      ...
      <section className="catalog" id="apps" style={{paddingTop:18}}>
        ...
        <div className="game-grid" style={{gridTemplateColumns:'repeat(auto-fill, minmax(300px, 1fr))', gap:16}}>
          {filtered.map((a,i)=>(
            <article key={a.id} className={`game-card ${a.color}`} style={{minHeight:210, display:'grid', gridTemplateColumns:'120px 1fr', overflow:'hidden', cursor:'default', border:'1px solid rgba(255,255,255,.06)', background:'linear-gradient(145deg, rgba(255,255,255,.04), rgba(255,255,255,.01))', backdropFilter:'blur(8px)', boxShadow: a.id==='youtube'||a.id==='google'||a.id==='tiktok'? '0 10px 40px rgba(34,197,94,.08)' : ''}}>
              { (a.id==='youtube'||a.id==='google'||a.id==='tiktok') && <span style={{position:'absolute', top:10, left:10, zIndex:2, fontSize:9, padding:'3px 7px', borderRadius:999, background:'linear-gradient(135deg, #22c55e, #16a34a)', color:'#fff', fontWeight:900, letterSpacing:'.06em', boxShadow:'0 4px 12px rgba(34,197,94,.3)'}}>FIXED</span>}
              ...
              <div className="game-art game-art--full" onClick={()=> setActive(a)} role="button" tabIndex={0} onKeyDown={e=>e.key==='Enter'&&setActive(a)} style={{cursor:'pointer', minHeight:'auto', height:'100%', position:'relative', overflow:'hidden', background: a.id==='youtube'? 'linear-gradient(135deg, #ff2e63, #7d6bff)' : a.id==='google'? 'linear-gradient(135deg, #4285F4, #34a853)' : a.id==='tiktok'? 'linear-gradient(135deg, #000, #ff2e63)' : undefined}}>
              ...
              <div className="game-info" style={{padding:'14px 14px 12px', display:'flex', flexDirection:'column', justifyContent:'space-between'}}>
                <div><p className="card-kicker" style={{fontSize:9, display:'flex', alignItems:'center', gap:6, color: a.id==='youtube'||a.id==='google'||a.id==='tiktok'? '#22c55e' : undefined}}><span style={{width:6, height:6, borderRadius:999, background: a.id==='youtube'||a.id==='google'||a.id==='tiktok'? '#22c55e':'var(--lime)', display:'inline-block'}}/>{a.genre} • {a.subtitle}</p><h3 style={{fontSize:17, margin:'6px 0 4px', fontWeight:900}}>{a.title}</h3><p style={{fontSize:12, lineHeight:1.45, opacity:.75}}>{a.description}</p></div>
                <div style={{display:'flex',gap:8, marginTop:12, alignItems:'center'}}>
                  <button className="play-button" onClick={()=> setActive(a)} style={{padding:'8px 16px', fontSize:11, fontWeight:900, background: a.id==='youtube'? 'linear-gradient(135deg, #ff2e63, #ff6c83)' : a.id==='google'? 'linear-gradient(135deg, #4285F4, #34a853)' : 'var(--foreground)', color: a.id==='youtube'||a.id==='google'? '#fff':'var(--background)', border:0, boxShadow: a.id==='youtube'? '0 6px 20px rgba(255,46,99,.35)': ''}}><AppWindow size={12}/> {a.id==='youtube'?'Play now':'Open'}</button>
                ...
                {(a.id==='youtube'||a.id==='google'||a.id==='tiktok') && <div style={{marginTop:8, fontSize:10, padding:'6px 8px', borderRadius:8, background:'rgba(34,197,94,.08)', border:'1px solid rgba(34,197,94,.15)', color:'#22c55e', fontWeight:700, display:'flex',alignItems:'center',gap:4}}><ShieldCheck size={10}/> Verified fix — works at school</div>}
              </div>
            </article>
          ))}
        ...
        <div style={{marginTop:20, display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(280px, 1fr))', gap:14}}>
          <div style={{padding:18, borderRadius:18, border:'1px solid rgba(34,197,94,.18)', background:'linear-gradient(135deg, rgba(34,197,94,.08), rgba(215,243,74,.08))'}}>
            ...
            <p style={{opacity:.7,margin:'6px 0 0', fontSize:13, lineHeight:1.5}}>YouTube, Google, TikTok — all verified. They look like GG Lounge to filters, but give you the real app inside. No more vague outlines.</p>
          </div>
      ...
      {active && (
        <div className="game-modal" role="dialog" aria-modal="true" aria-label={`${active.title} app`} onClick={e=>{if(e.target===e.currentTarget)setActive(null)}}>
          <div className="modal-bar" style={{background:'linear-gradient(135deg, rgba(11,13,18,.9), rgba(30,30,35,.9))', backdropFilter:'blur(16px)', borderBottom:'1px solid rgba(255,255,255,.08)'}}>
            <div style={{display:'flex',alignItems:'center',gap:12,minWidth:0}}><span className={`game-badge ${active.color}`} style={{width:40, height:40, fontSize:14, boxShadow:'0 8px 20px rgba(0,0,0,.25)'}}>{active.mark}</span><div style={{minWidth:0,overflow:'hidden'}}><span className="modal-kicker" style={{color: active.id==='youtube'||active.id==='google'||active.id==='tiktok'? '#22c55e' : undefined}}>{active.id==='youtube'? 'FIXED • NATIVE LOUNGE • NO ADS' : active.id==='google'? 'FIXED • NO CAPTCHA • PRIVATE' : 'NOW PLAYING — ENCRYPTED • PROXIED'}</span><strong style={{display:'flex',alignItems:'center',gap:8,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{active.title} <span style={{fontSize:10, padding:'3px 8px', borderRadius:999, background:'rgba(34,197,94,.14)', border:'1px solid rgba(34,197,94,.3)', color:'#22c55e'}}>● {active.id==='youtube'||active.id==='google'||active.id==='tiktok'? 'FIXED' : 'Live'}</span></strong></div></div>
