...
import { canonicalGameTitle } from "@/lib/game-identity";
import { MAX_ICON_BYTES, getRequest, createRequest, createSimpleRequest, listRequests, upvoteRequest, validateGameHtml, checkUpvoteCooldown } from "@/lib/db";
...
export async function POST(request: Request) {
  try {
    ...
    if (typeof (body as any).upvoteId === "string" && (body as any).upvoteId.trim()) {
      ...
      const selected = await getRequest(id);
      if (!selected) return NextResponse.json({ error: 'Not found' }, { status: 404 });
      const cd = await checkUpvoteCooldown(ip, canonicalGameTitle(selected.title) + ':' + selected.status, 30000)
      if (!cd.allowed) {
    ...
    if (!html) {
      ...
      return NextResponse.json({ ok: true, id: saved.id, request: { id: saved.id, title: saved.title, votes: saved.votes ?? 1, status: saved.status } });
    }
