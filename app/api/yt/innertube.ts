...
async function call(
  ...
  const attempt = async (ctx: Ctx): Promise<ProviderResult & { client?: string }> => {
    ...
    const res = await postJson(url, payload, timeoutMs, {
      // googlevideo binds the returned URLs to the requesting agent, so the
      // player call and the media relay must use the exact same user agent.
      "User-Agent": clientUa(String(ctx.client.clientName)),
...
export type NormalisedStreams = {
  ...
  provider: string;
  /** InnerTube identity that produced these URLs (media relay replays its UA). */
  ggClient?: string;
};
...
export async function streams(videoId: string): Promise<ProviderResult> {
  ...
  // which identity handed us these URLs - the media relay replays it
  built.data.ggClient = String(res.client || "WEB");
