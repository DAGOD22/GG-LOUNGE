'use client'
import { requestedReleases } from '@/lib/requested-releases'
  ...
  return (
    <div style={{marginTop:18,border:'1px solid var(--line)',borderRadius:16,background:'var(--panel)',padding:14}}>
      ...
      <details style={{marginTop:12,fontSize:12,color:'var(--muted)'}}>
        <summary style={{cursor:'pointer'}}>Requested local releases: file status</summary>
        <p>Installed editions are identified below. Other titles still need authorized HTML5/WebGL files, not just a saved portal page. Native desktop installers cannot be served as browser games.</p>
        <ul>{requestedReleases.map(release=><li key={release.title}><strong>{release.title}</strong> — {release.status}</li>)}</ul>
      </details>
      {requests.length>0 ? (
