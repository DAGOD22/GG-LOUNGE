...
export function GameModal({
  ...
  useEffect(() => () => { if (inspectionTimer.current) clearTimeout(inspectionTimer.current) }, [])
      ...
      if (Object.keys(ls).length === 0) {
        ...
        if (docLs) {
          for (let i = 0; i < docLs.length; i++) {
            const k = docLs.key(i)
            if (k && k !== 'ggl_customization_v1') ls[k] = docLs.getItem(k) || ''
          }
        }
      }
  ...
  const applySaveToFrame = useCallback((dataStr: string) => {
    ...
    try {
      ...
      for (const [k, v] of Object.entries(ls)) {
        if (k === 'ggl_customization_v1') continue
        try {
  ...
  useEffect(()=>{
    ...
    fetch(game.path, { method:'HEAD' }).then(r=>{
      ...
      if (!r.ok) { setFrameLoading(false); setFrameError(`Game files could not be loaded (HTTP ${r.status}).`); return }
      if (/DENY/i.test(xfo) || (/SAMEORIGIN/i.test(xfo) && new URL(game.path, location.href).origin !== location.origin)) setXFrameBlocked(true)
      else if(/frame-ancestors\s+[^;]*'none'/i.test(csp)) setXFrameBlocked(true)
    }).catch(()=>{})
    return ()=>{ alive=false }
  }, [game.path])
  const handleFrameLoad = useCallback(() => {
    if(xFrameBlocked){ setFrameLoading(false); setFrameError('This game blocks embedding (X-Frame-Options). Try opening it in a new tab.'); return }
    ...
    try {
      ...
      if (!doc.querySelector('style[data-ggl-patch]')) {
        ...
        style.textContent = `
        html,body{margin:0!important;overscroll-behavior:none!important}
        /* Game engines own their canvas size and loader visibility. */
      `
      ...
      // PROD X-Frame detection: immediate null doc check + hardened 2200ms fallback
      try {
        // immediate signal: cross-origin block gives null document or throws on access
        if (!doc || !doc.body) {
          setFrameError('This game blocks embedding (X-Frame-Options). Try opening it in a new tab.')
          return
        }
        try { void doc.body.innerHTML } catch { setFrameError('This host forbids iframes (X-Frame-Options). Use “Open in new tab”.'); return }
      } catch {}
      ...
      inspectionTimer.current = setTimeout(() => {
        try {
          ...
          if (blocked) { setFrameError('This game blocks embedding (X-Frame-Options). Try opening it in a new tab.'); return }
          ...
          if (!hasCanvas && !hasGame && bodyChildren === 0) setFrameError('The game looks empty — its files may be blocked on this network. Try opening in a new tab or via Proxy.')
          else if (!hasCanvas && !hasGame && /404|Failed to download|NOT FOUND|cannot fetch|NetworkError|refused/i.test(txt) && txt.length < 2500 && txt.length > 20) setFrameError('This game didn’t load. Try again or open in a new tab.')
          // Slow engine boot is not evidence of X-Frame-Options blocking.
        } catch {}
      }, 2200)
    } catch {}
  }, [applySaveToFrame, xFrameBlocked])
